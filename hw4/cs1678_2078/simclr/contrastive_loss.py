...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # compute l(k, k+N)
        numer = torch.exp(sim(z_k, z_k_N) / tau)
        denom = 0
        for i in range(2*N):
            if i != k:
                denom += torch.exp(sim(z_k, out[i]) / tau)

        total_loss -= torch.log(numer / denom)

        # compute l(k+N, k)
        numer = torch.exp(sim(z_k_N, z_k) / tau)
        denom = 0
        for i in range(2*N):
            if i != k + N:
                denom += torch.exp(sim(z_k_N, out[i]) / tau)
...
def sim_positive_pairs(out_left, out_right):
    ...
    dot_product = torch.sum(out_left * out_right, dim=-1, keepdims=True)
...
def compute_sim_matrix(out):
    ...
    dot_product = torch.matmul(out, out.T)
    norm = torch.linalg.norm(out, dim=-1, keepdims=True)
    sim_matrix = dot_product / (norm * norm.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    mask = (
        torch.ones_like(exponential, device=device) - 
        torch.eye(2 * N, device=device)
    ).to(device).bool()
